...
Минимизировать $z=\max\{|x_1-x_2+3x_3|,|-x_1+3x_2-x_3|\}$, при ограничениях $x_1,x_2,x_3 \geq 0$. Подсказка: представьте, что $z$ это число, которое не меньше, чем аргументы $\max$ и примените свойство модуля $|a| \leq b$ эквивалентна записи $a\leq b$ и $a \geq -b$. Привести к линейной модели. <!--Таха 3.5 с. 139-->
...
Обозначим переменные — сколько часов в неделю активист тратит на каждую деятельность:

### Ограничения

Всего свободного времени — 70 часов, значит:

$$
t_1+t_2+t_3+t_4=70
$$

На учёбу нужно тратить не меньше 6 часов в день 6 дней в неделю:

$$
t_1 \geq 36
$$

Тусовок организуется в два раза меньше, чем мероприятий:

$$
t_4 = \frac{1}{2}t_3
$$

Естественно время положительное
...
Примем удовольствие от часа учёбы за единицу:

$$
a_1=1
$$

Мероприятия интереснее учёбы в 2 раза, значит:

$$
a_3=2
$$

Тусовки в два раза круче мероприятий, значит:

$$
a_4=4
$$

Тусовки в 1.5 раза лучше, чем спорт, значит:
...
$$
a_2=\frac{4}{1.5}=\frac{8}{3}
$$

Тогда целевая функция максимизации удовольствия:

$$
F = t_1+\frac{8}{3}t_2 + 2 t_3 + 4 t_4 \to \max
$$

Итоговая модель линейного программирования:

$$
\begin{aligned}
&F = t_1+\frac{8}{3}t_2 + 2t_3 + 4t_4 \to \max \\
&\begin{cases}
t_1+t_2+t_3+t_4 = 70 \\
t_1 \ge 36 \\
t_4 = \frac{1}{2}t_3 \\
